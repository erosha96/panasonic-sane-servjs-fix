...
do
  {
    ...
    /* Read data from scanner if buffer is empty and status is good */
    if (status == SANE_STATUS_GOOD && (timeout || FD_ISSET(be_fd, &rd_set)))
      {
        /* Reserve 4 bytes for record length */
        size_t reader = 4;
        nbytes = buffer_size - 4; /* Leave space for length prefix */

        DBG (DBG_INFO, "do_scan: trying to read %zu bytes from scanner\n",
             nbytes);
        status = sane_read (handle[h].handle, buf + reader, nbytes, &length);
        ...
        if (status == SANE_STATUS_GOOD)
          {
            store_reclen (buf, buffer_size, 0, length); /* Store length at start */
          }
        else
          {
            length = 0; /* Reset length since no valid data was read */
            status_dirty = 1;
    ...
    /* Write data to client if there’s data to send */
    if (FD_ISSET(data_fd, &wr_set) && length > 0)
      {
        nbytes = length + 4; /* Include length prefix */
        DBG (DBG_INFO, "do_scan: trying to write %zu bytes to client\n",
             nbytes);
        nwritten = write (data_fd, buf, nbytes);
        ...
        length = 0; /* Clear buffer by resetting length */
      }

    /* Append status code if needed */
    if (status_dirty && buffer_size >= 5)
      {
        status_dirty = 0;
        store_reclen (buf, buffer_size, 0, 0xffffffff); /* Special length */
        buf[4] = status; /* Status code after length */
        nbytes = 5;

        if (FD_ISSET(data_fd, &wr_set))
          {
            DBG (DBG_INFO, "do_scan: sending statuscode `%s' to client\n",
                 sane_strstatus (status));
            nwritten = write (data_fd, buf, nbytes);
            if (nwritten < 0)
              {
...
while (status == SANE_STATUS_GOOD || status_dirty);
